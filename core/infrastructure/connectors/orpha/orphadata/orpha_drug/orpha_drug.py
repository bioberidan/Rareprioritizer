...
class DrugParser:
    ...
    def parse_drugs(self, soup: BeautifulSoup) -> List[Dict]:
        """Parse drug information from search results."""
        drugs = []
        
        # Try multiple strategies to find drug containers
        drug_containers = self._find_drug_containers(soup)
        
        for container in drug_containers:
            try:
                drug_data = self._parse_single_drug(container)
                if drug_data and drug_data.get('name'):
                    drugs.append(drug_data)
                    
            except Exception as e:
                self.logger.warning(f"Failed to parse drug container: {e}")
                continue
        
        return drugs
    
    def _find_drug_containers(self, soup: BeautifulSoup) -> List:
        """Find containers that hold drug information."""
        # Strategy 1: Look for Orpha.net specific drug-card structure
        drug_cards = soup.find_all('div', class_='drug-card')
        if drug_cards:
            return drug_cards
        
        # Strategy 2: Look for the specific class combination used by Orpha.net
        containers = soup.select('div.row.drug-card.bg-gray')
        if containers:
            return containers
        
        # Strategy 3: Look for containers with substance links
        containers_with_substance = []
        all_divs = soup.find_all('div', class_='m-4')
        for div in all_divs:
            if div.find('a', href=lambda x: x and 'substance' in x):
                containers_with_substance.append(div)
        if containers_with_substance:
            return containers_with_substance
        
        # Strategy 4: Look for table rows (fallback)
        table_rows = soup.find_all('tr')
        if len(table_rows) > 1:  # Skip header row
            return table_rows[1:]
        
        # Strategy 5: Generic containers with links
        containers = soup.find_all('div', class_=['item', 'entry', 'record'])
        return containers
