...
from .models import GroupsResponse, GroupsResponseV3
...
@register_prompt
class GroupsPromptV3(BasePrompt):
    """
    Version 2 of the CIBERER groups analysis prompt.
...
- Check ResearchGate, LinkedIn for CIBERER + {disease_name} connections
- Scan clinical trial registries for CIBERER involvement
- The above search terms are just examples, you can use other search terms if needed, but result should has a clear connection to the disease and CIBERER
...
{{
  ...
  "groups": [
    {{
      ...
      "sources": [
        {{"label": " if source is pubmed, use the article title, otherwise use the source description", "url": "verifiable URL"}}
      ]
...
**QUALITY REQUIREMENTS:**
✓ Confirm the exact spelling of the group name appears verbatim in at least one source (webpage, PDF, publication)
✓ Minimum 1 verifiable source per unit
✓ Verify all PMIDs resolve correctly
✓ Use empty arrays [] if no data found
✓ Unit name formats: Prefer unit names beginning with U xxx; otherwise keep the full descriptive (e.g Grupo de enfermedades raras neurodegenerativas) or PI-based label (e.g "grupo del Dr. Perez") exactly as written.
✓ Return pure JSON (no markdown formatting)

**EXAMPLES:**
- example of non pubmed, source description:
        {{
          "label": "CIBERER news: Revisan el papel de la vía de pigmentación de la retina en el albinismo humano",
          "url": "https://www.ciberer.es/noticias/revisan-el-papel-de-la-via-de-pigmentacion-de-la-retina-en-el-albinismo-humano"
        }}
-examples of different unit name formats:
Uxx format: U746, U123, U456
Descriptive label: "Grupo de enfermedades raras neurodegenerativas", "Unidad de albinismo"
PI-based label: "grupo del Dr. Perez", "grupo de Mara Diaz"
...
**Instructions**: Return the JSON object with comprehensive CIBERER unit data."""
    ...
    @property
    def model(self) -> Type[GroupsResponseV3]:
        return GroupsResponseV3
